...
public class Principal3 {
	public static void main(String [] args) {
		...
		Predicate<Order> isCategoryToys = orderItem -> orderItem.getProducts()
															.stream() // Predicate: boolean para testar se o produto atual é da categoria Baby
															.anyMatch(category -> category.getCategory().equalsIgnoreCase("Toys"));
		
		Function<Order, List<Product>> discount10 = orderItem -> orderItem.getProducts()
															.stream()
		...
		List<Product> productsCategoryBaby = orderList1.stream()
														.filter(isCategoryToys) // Aplicar a lógica
														.flatMap(order -> discount10.apply(order).stream())
														.collect(Collectors.toList());
